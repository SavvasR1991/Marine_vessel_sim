...
def plot_graph(t, values, area):
    ...
    count = 1
    for i in t :   
        if float(values[count-1][0]) != 0 or float(values[count-1][1]) != 0 or float(values[count-1][2]) != 0:
            print("Area : " + str(count))
            print("	Frequency: " + values[count-1][0] + " seconds")
            print("	Direction: " + values[count-1][1] + " degrees")
            print("	Height:    " + values[count-1][2] + " meters\n")
        tmp = []
        points = []
        for j in i:
...
def main():
    ...
    for i in Lines:
        ...
        values .append([i.split()[8],i.split()[9],i.split()[10]])
    plot_graph(squares, values, area)
