...
namespace Surveilance_Loiter {
	...
	void patroling(VESSEL_KINEMATICS *vessel_kinematics, VESSEL_CHARACTERISTICS* vessel_char, VESSEL_MOTION_VALUES *vessel_motion, list<Point*>::iterator &current_patroling_point, std::list<Point*> &patroling_points, int &counter_patrol) {
		...
		if (vessel_char->get_type() == "SURFACE") {
			...
			if (dist_curr_to_patroling_target <= 50) {
			...
			vessel_kinematics->set_safe_xyz((*current_patroling_point)->x(), (*current_patroling_point)->y(), 0);
			...
			if ((dist_curr_to_patroling_target <= 25) && (patroling_target_z <= (vessel_kinematics->get_current_z() + vessel_char->get_height()))) {
				if ((current_patroling_point != patroling_points.end()) && (next_iteration(current_patroling_point) == patroling_points.end()))
				{
					current_patroling_point = patroling_points.begin();
				}
	...
	/******************* RESET **********************/
	void reset(VESSEL_KINEMATICS *vessel_kinematics, VESSEL_CHARACTERISTICS* vessel_char,list<Point*>::iterator current_patroling_point, std::list<Point*> patroling_points,bool *reset_mode_on, char* job) {

		double curr_x = vessel_kinematics->get_current_x();
		double curr_y = vessel_kinematics->get_current_y();

		double reset_target_x = vessel_kinematics->get_reset_x();
		double reset_target_y = vessel_kinematics->get_reset_y();
		...
		if (vessel_char->get_type() == "SURFACE") {
			...
			if (dist_curr_to_reset_target < vessel_char->get_area_radius() / 2) {
				current_patroling_point = patroling_points.begin();
				vessel_kinematics->set_destination_xyz((*current_patroling_point)->x(), (*current_patroling_point)->y(), 0);
				vessel_kinematics->set_destination_theta_horizontal(theta_dest_horiz);
				*reset_mode_on = false;
			...
			if (dist_curr_to_reset_target <= vessel_char->get_area_radius()) {
				current_patroling_point = patroling_points.begin();
				double theta_dest_vetical = angle_between_3d_points(curr_x, curr_y, curr_z,	(*current_patroling_point)->x(), (*current_patroling_point)->y(), (*current_patroling_point)->z());
				vessel_kinematics->set_destination_xyz((*current_patroling_point)->x(), (*current_patroling_point)->y(), (*current_patroling_point)->z());
				vessel_kinematics->set_destination_theta_horizontal(theta_dest_horiz);
